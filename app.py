...
doj_transformer = Pipeline(
    steps=[
        (
            "dt",
            DatetimeFeatures(
                features_to_extract=feature_to_extract, yearfirst=True, format="mixed"
            ),
        ),
        ("scaler", MinMaxScaler()),
    ]
...
time_transformer = FeatureUnion(
    transformer_list=[("part1", time_pipe1), ("part2", time_pipe2)]
)
...
duration_pipe1 = Pipeline(
    steps=[("rbf", RBFPercentileSimilarity()), ("scaler", PowerTransformer())]
)
...
total_stops_transformer = Pipeline(
    steps=[
        ("imputer", SimpleImputer(strategy="most_frequent")),
...
selector = SelectBySingleFeaturePerformance(
    estimator=estimator, scoring="r2", threshold=0.1
)
...
st.title("Flight Price Predictor - AWS  Sagemaker")

# inputs box
airline = st.selectbox("Airline:", train.airline.unique())

date_of_journey = st.date_input("Date of Departature")

source = st.selectbox("Source", x_train.source.unique())

destination = st.selectbox("Destination", train.destination.unique())
...
additional_info = st.selectbox("Additional Information", train.additional_info.unique())


# converting input values in dataframe
x_new = pd.DataFrame(
    dict(
        airline=[airline],
...
# here the pandas datatime object is not work with streamlit datetime object(during transformation)
# so we need to convert the streamlit date time object into string which can be then accepted by the pandas date time object
if st.button("Predict"):
    # loading the preprocessor to preprocessed the dataframe
    saved_preprocessor=joblib.load("preprocessor.joblib")
    x_new_pre=saved_preprocessor.transform(x_new)

    #importing the model
    with open("./models/xgboost-model","rb") as f:
        model=pickle.load(f)
